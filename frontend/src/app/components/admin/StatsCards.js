...
export default function StatsCards({ stats, activeTab }) {
  ...
  const cardGroups = [
    ...
    },
    {
      ...
      color: 'green',
      tabs: ['internships', 'internship-enrollments'],
      cards: [
        {
    ...
    },
    {
      ...
      color: 'purple',
      tabs: ['projects', 'project-payments'],
      cards: [
        {
  ...
  // Filter card groups based on active tab
  const visibleGroups = cardGroups.filter(group =>
    group.tabs.includes(activeTab)
  );

  // Show revenue card only on payment-related tabs
  const showRevenue = ['payments', 'project-payments', 'internship-enrollments'].includes(activeTab);

  return (
    <div className="space-y-6 mb-8">
      {/* Revenue Card - Show only on payment tabs */}
      {showRevenue && (
        <div className="">
      ...
      {/* Grouped Cards - Only show relevant groups */}
