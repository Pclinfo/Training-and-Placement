...
# # Replace the create_course endpoint with this:
# @app.route("/admin/courses", methods=["POST"])
# @jwt_required()
# def create_course():
#     try:
...
# # Replace the update_course endpoint with this:
# @app.route("/admin/courses/<int:course_id>", methods=["PUT"])
# @jwt_required()
# def update_course(course_id):
#     try:
...
#         if 'features' in data:
#             try:
#                 course.features = json.loads(data['features'])
#             except json.JSONDecodeError:
#                 pass
...
# # UPLOAD_FOLDER = os.path.join(BASE_DIR, "uploads")
# # os.makedirs(UPLOAD_FOLDER, exist_ok=True)
# # After BASE_DIR and FRONTEND_DIR definitions
# UPLOAD_FOLDER = os.path.join(BASE_DIR, "uploads")
# COURSE_IMAGES_FOLDER = os.path.join(UPLOAD_FOLDER, "courses")

# # Create directories
# os.makedirs(UPLOAD_FOLDER, exist_ok=True)
# os.makedirs(COURSE_IMAGES_FOLDER, exist_ok=True)
# os.makedirs(FRONTEND_DIR, exist_ok=True)
...
# def save_course_image(file_obj):
#     """Save course image and return the URL path"""
#     if not file_obj or file_obj.filename == "":
#         return None
    
#     if not allowed_file(file_obj.filename):
#         app.logger.error(f"Invalid file type: {file_obj.filename}")
#         return None
...
#     safe_name = f"course_{timestamp}_{unique_id}.{ext}"
    
#     # Save file
#     file_path = os.path.join(COURSE_IMAGES_FOLDER, safe_name)
#     file_obj.save(file_path)
    
#     app.logger.info(f"Image saved: {safe_name}")
    
#     # Return URL path (not file system path)
#     return f"/uploads/courses/{safe_name}"

# def delete_course_image(image_url):
#     """Delete course image file"""
#     if not image_url or not image_url.startswith('/uploads/courses/'):
#         return False
...
#             os.remove(file_path)
#             app.logger.info(f"Image deleted: {image_url}")
#             return True
#     except Exception as e:
...
# #         base_slug = data.get('slug') or data.get('title', '').lower().replace(' ', '-')
# #         slug = base_slug
# #         counter = 1
# #         while Course.query.filter_by(slug=slug).first():
# #             slug = f"{base_slug}-{counter}"
...
# #         return jsonify({'success': True, 'course_id': course.id, 'slug': course.slug}), 201
...
# #         for field in ['title', 'description', 'detailed_description', 'level', 'students', 
# #                      'duration', 'price', 'original_price', 'discount', 'image_url', 
# #                      'category', 'instructor', 'course_fees', 'total_amount', 'features']:
# #             if field in data:
# #                 if field == 'rating':
...
#                 image_url = save_course_image(file)
#                 if not image_url:
#                     return jsonify({'error': 'Invalid image file. Allowed: JPG, PNG, GIF, WebP (Max 5MB)'}), 400
...
#                 features = json.loads(data['features'])
#             except json.JSONDecodeError:
#                 app.logger.warning("Failed to parse features JSON")
#                 features = []
...
#             original_price=data.get('original_price'),
#             discount=data.get('discount'),
#             image_url=image_url or data.get('image_url'),  # Use uploaded or provided URL
#             category=data.get('category'),
#             instructor=data.get('instructor'),
#             slug=slug,
#             course_fees=data.get('course_fees'),
...
#         app.logger.info(f"Course created: {course.id} - {course.title}")
...
#             file = request.files['image']
#             if file.filename:  # Check if file was actually selected
#                 new_image_url = save_course_image(file)
#                 if new_image_url:
...
#         updateable_fields = [
#             'title', 'description', 'detailed_description', 'level', 
#             'students', 'duration', 'price', 'original_price', 'discount', 
#             'category', 'instructor', 'course_fees', 'total_amount'
#         ]
...
#         # Handle rating separately (needs float conversion)
#         if 'rating' in data:
#             course.rating = float(data['rating'])
...
#         app.logger.info(f"Course updated: {course.id} - {course.title}")
...
#         app.logger.info(f"Course deleted: {course.id} - {course.title}")
...
#     return send_from_directory(COURSE_IMAGES_FOLDER, filename)
